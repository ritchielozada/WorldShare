...
/// </summary>
public class ImportExportAnchors : Singleton<ImportExportAnchors>
{
    /// <summary>
    ...
    void Start()
    {
        Debug.Log("ImportExportAnchors - Start()");
        DisplayUI.Instance.AppendText("ImportExportAnchors - Start()");
    ...
    private void RoomManagerCallbacks_AnchorUploaded(bool successful, XString failureReason)
    {
        ...
        {
            DisplayUI.Instance.AppendText("Upload failed " + failureReason);
    ...
    {
        anchorStore = store;
        DisplayUI.Instance.AppendText("ImportExportAnchors-AnchorStoreReady()");
    ...
    {
        DisplayUI.Instance.AppendText("Instance_SessionJoined()");
    ...
    {
        DisplayUI.Instance.AppendText("SharingServiceReady");
    ...
    {
        long localUserId = AnchorShareCustomMessage.Instance.localUserID;
    ...
    void StartAnchorProcess()
    {
        ...
        DisplayUI.Instance.AppendText("StartAnchorProcess() - Found " + anchorCount + " anchors");
        ...
        if (anchorCount > 0)
        {
            ...
            {
                DisplayUI.Instance.AppendText("Starting room download");
    ...
    void MakeAnchorDataRequest()
    {
        ...
        {
            DisplayUI.Instance.AppendText("Couldn't make the download request.");
            currentState = ImportExportState.Failed;
        }
    }
    ...
    private void Anchor_OnTrackingChanged_InitialAnchor(WorldAnchor self, bool located)
    {
        if (located)
        {
            DisplayUI.Instance.AppendText("Found anchor, ready to export");
            currentState = ImportExportState.ReadyToExportInitialAnchor;
        }
        else
        {
            DisplayUI.Instance.AppendText("Failed to locate local anchor (super bad!)");
            currentState = ImportExportState.Failed;
        }
    ...
    {
        DisplayUI.Instance.AppendText("Looking for " + AnchorName);
        string[] ids = anchorStore.GetAllIds();
        for (int index = 0; index < ids.Length; index++)
        {
            if (ids[index] == AnchorName)
            {
                DisplayUI.Instance.AppendText("Using what we have");
    ...
    {
        DisplayUI.Instance.AppendText("anchor " + located);
        if (located)
        {
            currentState = ImportExportState.Ready;
        }
        ...
        {
            DisplayUI.Instance.AppendText("Failed to find local anchor from cache.");
            MakeAnchorDataRequest();
        }
    ...
    void ImportComplete(SerializationCompletionReason status, WorldAnchorTransferBatch wat)
    {
        ...
        {
            DisplayUI.Instance.AppendText("Import complete");

            string first = wat.GetAllIds()[0];
            DisplayUI.Instance.AppendText("Anchor name: " + first);
        ...
        {
            DisplayUI.Instance.AppendText("Import fail");
            currentState = ImportExportState.DataReady;
        }
    }
    ...
    public void ExportComplete(SerializationCompletionReason status)
    {
        ...
        {
            DisplayUI.Instance.AppendText("Uploading anchor: " + exportingAnchorName);
            roomManager.UploadAnchor(
                currentRoom,
                new XString(exportingAnchorName),
                exportingAnchorBytes.ToArray(),
                exportingAnchorBytes.Count);
            DisplayUI.Instance.AppendText("Anchor Export Completed");
        }
        else
        {
            DisplayUI.Instance.AppendText("This anchor didn't work, trying again");
